...
namespace muduo
{
	namespace net
	{
		...
		private:
			...
			bool retry_;   // atomic	// 重连,是指连接建立之后又断开的时候是否重连
			bool connect_; // atomic
			// always in loop thread
			int nextConnId_;		// name + nextConnId_用于标识一个连接
			mutable MutexLock mutex_;
			TcpConnectionPtr connection_ GUARDED_BY(mutex_);	// Connector连接成功后，得到一个TcpConnection
		};
