...
namespace
{
	//定义一个全局变量指针，该指针在Inspector的构造函数中进行赋值，指向他自己
...
#endif
	// 注册完函数后，调用start()函数
	loop->runAfter(0, std::bind(&Inspector::start, this)); // little race condition竞态问题
  /*
  使用runAfter启动start函数，是为了防止竞态问题
  如果直接调用start()，(当线程不是IO线程也可能时IO线程，只不过不是当前所属EventLoop的IO线程，是主线程)，那么有可能，当前构造函数还没有返回，
  HttpServer所在的IO线程可能已经收到了http客户端的请求了(因为这时候HttpServer已启动)，
  那么就会回调Inspector::onRequest进行处理，而这时候构造函数还没返回，也就是说对象还没有创建成功我们就调用了onRequest

  作者的处理是在构造函数之后，在loop中延迟(0秒，即立即执行)调用start函数，也就是在httpServer所在的IO线程中调用start函数
  */
...
// 使用如下:add("proc", "pid", ProcessInspector::pid, "print pid");
// http://192.168.10.130:12345/proc/pid这个http请求就会响应的调用ProcessInspector::pid来处理
void Inspector::add(const string& module,//模块
	const string& command,//命令
	const Callback& cb,//命令的回调函数
	const string& help)//命令的说明帮助
{
	MutexLockGuard lock(mutex_);

	//modules_[module]取出map中的value，value又是一个map，再用[]取出map中的value
	modules_[module][command] = cb;//modules_是map容器，key是模块名，value是一个map，在用[]取出map中的value
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse* resp)
{
		...
		*/
			...
			++helpListI)
		{
			...
			for (const auto& it : list)
			{
				...
				result += string(len >= 25 ? 1 : 25 - len, ' ');//制表符，使得能够对齐
				result += it.second;
				result += "\n";
			}
		}
	...
	else
	{
		...
		//result==1，说明，只分割了一个模块，/proc
		else if (result.size() == 1)
		{
			//只有module，没有command也是错的，因此ok认为false
		...
		else
		{
			...
			if (commListI != modules_.end())
			{
				...
				if (it != commList.end())//it!=end(),说明找到了
				{
					//存一下命令后面的参数，/proc/pid/1/2/3/4
					//						/1/2/3/4可以组成一个参数列表
					...
					if (it->second)
					{
						...
						resp->setBody(cb(req.method(), args));//回调函数进行调用，将返回的字符串传给setBody
						ok = true;
					}
				}
			}
