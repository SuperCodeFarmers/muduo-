...
void onRequest(const HttpRequest& req, HttpResponse* resp)
{
	...
	if (req.path() == "/")
	{
		...
		resp->setStatusMessage("OK");//设置状态
		resp->setContentType("text/html");//设置相应实体的媒体类型
...
int main(int argc, char* argv[])
{
	...
	server.setHttpCallback(onRequest);//设置实例处理http实体的回调函数
	server.setThreadNum(numThreads);//设置线程数
	server.start();//启动服务器
	loop.loop();
}
